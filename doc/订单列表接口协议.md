...
| 失败通道  |  failChannel  |  IntArray  |  无  | [{"channelId":1,"channelTime":300}] | 失败通道的数组和时长，显示用："山东移动分省-鼎山（标准包", "山东移动分省-硕软" |
| 当前使用通道  |  currentChannel  |  Int  |  无  | {"channelId":1,"channelTime":300} | 当前正在使用的通道和时长，如：山东移动分省-大汉 |
| 未使用通道  |  unusedChannel  |  IntArray  |  无  | [3,4] | 未使用通道的数组，显示用："山东移动漫游-鼎山（标准包", "山东移动漫游-硕软" |
...
| 通道ID | channelId | int |  无  | 1 |    |
| 通道时长 | channelTime | int |  无  | 100 |  显示用：1分40秒  |
| 退款原因 | refundReason | String |  有  | 用户有业务互斥 |  在退款的时候填入退款原因，客户后台需同步显示  |
| TOB平台订单号  |  platformOrderId  | String |  有  | 2017072116237582375  |  TOB平台订单的唯一标识  |
    ...
    [
        {
            ...
            "remark": "我是一个备注2",
            "refundReason":"用户欠费",
            "platformOrderId":"20170809382748"
        }
    ]
}
```
